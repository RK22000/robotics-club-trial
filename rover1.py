...
class Rover:
    ...
    def huristic_path(self):
        ...
        q = [(0, random.random(), hashify(self.pos), None)] # cost, cur, prev
        popped = dict()
        while q:
            cost, _, cur, prev = heapq.heappop(q)
            ...
            for n in neighbors:
                ...
                if not c: continue
                heapq.heappush(q, (cost+c, random.random(), hashify(n), hashify(cur)))
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if len(hpath) > 1:
            pygame.draw.lines(screen, 'green', False, [cell_size*(i+(0.5, 0.5)) for i in hpath])
        if len(self.path) > 1:
